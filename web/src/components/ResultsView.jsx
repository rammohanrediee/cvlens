...
function OverviewPanel({ analysis, onReview }) {
  ...
  const passed = checks.filter(check => check.matched)
  const attention = checks.filter(check => !check.matched)
  return <div className="overview-grid">
    <section><h2>Your resume at a glance</h2><p className="panel-description">Structure checks, not a prediction of hiring success.</p>
      <dl className="overview-metrics">
        <div><dt>Pages</dt><dd>{analysis.candidate.page_count ?? '—'}</dd></div>
        <div><dt>Checks passed</dt><dd>{passed.length}<small> / {checks.length}</small></dd></div>
        <div><dt>Needs attention</dt><dd>{attention.length}</dd></div>
      </dl>
      <Breakdown sections={analysis.ats_section_scores || []} />
    </section>
    <section className="review-findings"><h3>Keep these strengths</h3>
      ...
      <h3>Focus next</h3><ul>{attention.length ? attention.slice(0, 3).map(check => <li key={check.label}><Icon name="alert" size={16} /><span>{check.label}</span></li>) : <li>Review the language in your experience section.</li>}</ul>
...
function AtsPanel({ analysis }) {
  ...
  return <div className="detail-panel">
    <header className="panel-heading"><h2>ATS structure check</h2><p>Check what the parser detected. This is not an employer ATS verdict.</p></header>
    {[{label:'Needs attention', items:checks.filter(check => !check.matched)}, {label:'Passed', items:checks.filter(check => check.matched)}].map(group =>
      <section className="check-group" key={group.label}><h3>{group.label} <span className="count">{group.items.length}</span></h3>
        {group.items.length ? group.items.map(check => <details className="check-disclosure" key={check.label}>
          ...
          <p>{check.matched ? check.success : check.warning}</p><small>Weight in structure score: {check.weight} points</small>
...
function SuggestionsPanel({ analysis, onPreview, drafts, setDrafts }) {
  ...
  return <div className="detail-panel">
    <header className="panel-heading"><h2>Make your experience count</h2><p>Work through the feedback. Keep only wording you can support.</p></header>
...
function KeywordsPanel({ analysis, onEdit }) {
  ...
  return (
    <div className="detail-panel">
      <div className="detail-panel__intro"><h2>How your experience fits the role.</h2><p>{analysis.requirement_evidence?.summary}</p></div>
...
export default function ResultsView({
  ...
  const missing = (analysis.ats_checks || []).filter(check => !check.matched).sort((a, b) => b.weight - a.weight)
  ...
  return <article className="report" aria-labelledby="results-title">
    <header className="report-heading">
      <div><p className="workflow-status">{activeTab === 'suggestions' ? 'Review · Improve your wording' : 'Review · Analysis complete'}</p><h1 id="results-title" ref={headingRef} tabIndex="-1">{file.name}</h1><p className="document-meta">PDF · {Math.ceil(file.size / 1024).toLocaleString()} KB · {analysis.candidate.page_count ?? '—'} {analysis.candidate.page_count === 1 ? 'page' : 'pages'}</p></div>
    ...
    <section className="result-summary" aria-label="Analysis summary">
      ...
      <div className="summary-priority"><span className="priority-label">Your next improvement</span><h2>{priority}</h2><p>{finding?.coaching_tip || missing[0]?.warning || 'Add a job description to find relevant requirements and missing skills.'}</p><button type="button" className="text-button" onClick={() => selectTab(nextTab, true)}>{nextTab === 'suggestions' ? 'Work on this bullet' : nextTab === 'ats' ? 'Inspect this check' : 'Tailor to a job'} <Icon name="arrow" size={16} /></button></div>
    </section>
    <div className="mobile-context-actions"><button type="button" className="secondary-button" onClick={() => setPreviewOpen(true)}>View resume</button><button type="button" className="secondary-button" onClick={() => roleProvided ? selectTab('keywords', true) : onEdit()}>Tailor to job</button></div>
    <div className="results-layout">
      ...
      <aside className="context-panel" aria-label="Resume context">
        ...
        <section className="role-tool"><h2>Tailor to a job</h2>{roleProvided ? <><p className="role-score">{scoreValue(analysis.summary.semantic_match_score) ?? '—'}<small>{analysis.summary.semantic_match_score == null ? ' Not available' : '% role similarity'}</small></p><p className="role-excerpt">{analysis.job_description}</p><button type="button" className="text-button" onClick={() => selectTab('keywords',true)}>Explore match <Icon name="arrow" size={16} /></button><button type="button" className="text-button" onClick={onEdit}>Edit job description</button></> : <><p>Add a role to compare requirements with your experience.</p><button type="button" className="secondary-button" onClick={onEdit}>Add job description</button></>}</section>
        <p className="context-note">Scores guide your review. They do not predict hiring decisions.</p>
      </aside>
    </div>
